...
function out = l1homotopy_elastic(A, y, opts)
...
delx = ((A(:,gamma_xh)'*A(:,gamma_xh)) + eye(size(A(:,gamma_xh),2)).*0.001)\rhs(gamma_xh);
...
while iter < maxiter
    ...
    update_delx;
         ...
         AtAgx = (A(:,gamma_xh)'*A(:,gamma_xh)) + eye(size(A(:,gamma_xh),2)).*0.0001;
         delx = AtAgx\rhs(gamma_xh); % -AtAgx\(dW(gamma_xh).*z_x);
         %delx = inv((A(:,gamma_xh)'*A(:,gamma_xh)) + eye(size(A(:,gamma_xh),2)).*0.001)*rhs(gamma_xh);
